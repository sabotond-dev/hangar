...
| Port the nine BOTOR presets as the seed catalog, then author new ones | They are produced by a tested compiler, they fit the budget, and several are hardware-proven; authoring from zero would re-litigate solved problems | ✓ Good — Phase 3: compiler and simulator vendored byte-near at `sabotond-dev/botor@a0fb69d5`, all nine presets character-identical to BOTOR's own compiler, simulator pinned against an independently derived firmware oracle (26/26) ; Phase 8: seven new instruments authored as hand-written Lua, animated by a wasmoon VM driving the same vendored LED engine (parity proven on all nine presets), each fitting 908/908 across its whole knob range — the audition on real hardware is the user's |
| Live `pad-sim` preview on every card | The hardware's own LED mirror is 3.3 Hz, so the simulator is the only possible preview — and it makes the site worth opening with no ZONA attached | ✓ Good — Phase 4: the front door is a coverflow of live pads running the vendored simulator on the exact compiled configs (534 unit / 21 browser tests, verified 5/5); four of the nine seed presets turned out quiet and one all-black — declared, gated, never faked |
| Tune knobs, not full authoring | "Playground" means turning knobs on great configs; a browser layer/zone editor is a different, much larger product | ✓ Good — Phase 5: 75 knobs across 16 configurations through one three-widget vocabulary, live preview under the hero, two 908 meters, SURPRISE ME proven never over budget (18,000 draws) and every reachable knob state proven in budget (32,852 states) |
| Shareable state in the URL via the base36 stamp, no accounts | Keeps the whole thing static and free, and the stamp format already exists and is versioned | ✓ Good — Phase 5: `/c/<id>#z.<format><payload>` with BOTOR's format for compiler entries and a HANGAR format for Lua entries, entry-consistency checked, COPY LINK with an iOS fallback, build-time OG images on every routed page (Discord unfurl unverifiable until the embargo lifts) |
