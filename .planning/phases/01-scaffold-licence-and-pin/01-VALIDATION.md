...
| Drift `.prettierrc` (add `"singleQuote": true`) and run `npx vitest run src/lib/format-parity.spec.ts` | 1-02-03 | **Non-zero** with the drifted config; **zero** once `.prettierrc` is restored byte-exactly. Both exit codes recorded in the SUMMARY. *(Amended by checkpoint decision B, 2026-09-02: the canary now asserts that HANGAR's formatter output is byte-identical to grid-editor's own formatter output, not that a copied BOTOR file is Prettier-clean — three of the four zona files are not clean upstream. The old mangle-the-copy check no longer discriminates: Prettier normalises the mangle away on both sides. `--ignore-path .prettierignore` is still load-bearing and still present, since `.tmp-format-parity/` is gitignored and Prettier >= 3.0 defaults `--ignore-path` to `[.gitignore, .prettierignore]`.)* |
