...
| 4-01-03 | 01 | 1 | IDENT-01 | unit + build | `npx vitest run --project server src/lib/ui/identity.spec.ts && npm run build` | identity `6 passed`; the mark carried into `build/index.html` (Vite inlines an SVG under 4096 bytes as a data URI rather than emitting an asset file — observed: 3,651 bytes, inlined) | ⬜ |
