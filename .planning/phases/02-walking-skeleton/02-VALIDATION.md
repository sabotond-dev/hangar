...
| Frame scanner cross-chunk retention | 2-01-02 | drop the retained partial - replace `this.buf = this.buf.slice(start)` with `this.buf = []` after each emitted frame (the lookback floor `Math.max(this.scanned, 3)` is decorative: removing it only starts the loop at negative `i - 3` indices, which never match, so tests 3 and 4 stay green) | framing.spec tests 2, 3 and 4 red (the split-chunk cases lose their first half); tests 1 and 5 stay green |
...
| Matcher heartbeat guard | 2-01-03 | DELETE the HEARTBEAT early return in `match.ts` entirely (moving it after rule 4 is inert: a HEARTBEAT/EXECUTE against a HEARTBEAT filter still reaches the relocated guard and returns "no") | match.spec test 1 red — rules 5-7 then return "ok" for a heartbeat, and a heartbeat must never resolve a waiter |
