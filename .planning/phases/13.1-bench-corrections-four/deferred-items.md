...
| 12  | **Round 4b's item 3 (BENCH-2026-09-12.txt, Round 4b): the Sandbox's header controls "all over the place"** - the Edit / Play switch with its helper line under it, `Save copy` mid-right, `Export as a file` far right with its two-line explanation under it; four controls at four heights. **CLOSED 2026-09-12 by the quick task after the gate (commit `775a2d4`, the code alone).** | Before: as the user's screenshot has it (the mode line under the switch, the explanation under the export's box and that box lifted above Save copy's by it, the boxes at their own heights); not measured before the edit. After, on the served build in chromium: the name's row `align-items: center` twice (the row and the name block) - Rename and both segments **184.2-228.2** at 1280 and 1440, the name 186.4-226 (PDF page 3: the name at y 207, the switch's boxes 186-226, centred on the name's line); the mode line the sub-line's second line (a `.lines` block, gap 4; section 8's persistent visible label kept, the PDF's switch has no helper); the export's explanation `sr-only` (still `aria-describedby`, the string unchanged); ONE toolbar row - Undo, Redo, Save copy, Export as a file all **303.7-347.7** at 1280 and 1440, **431.5-475.5** at 1024 on one row (the compact band's boxes at 10px padding and 8px gaps: 353 in 372; at the wide band's 16 / 12 they are 413 and would not fit). The two transient outcome lines (`saved to My configs.`, `Exported as …`) fold beside their buttons (`overflow-wrap: anywhere`, `min-inline-size: 0`; `SurfaceActions`'s `.share` is `display: contents` so the export's line shrinks by its own width, not its button's) - one line at 1440 / 1920, two at 1280, the row still 44; both at once make the row 56 for their four seconds; under 480 of column (a container query on `.sandbox`) the right pair drops to a second line for those seconds. Document 720 / 720 and 900 / 900 in every state. | The name row at 1024 wraps - the 40px name is 343 wide in the compact band's 372 column, so the name folds to two lines and the switch takes its own line right-aligned (`margin-inline-start: auto`); IDENT-01's scale (D-17), not this row's. Both outcome lines at once at 1024 fold to five and six lines for four seconds - the rarest state at a width the bench does not use; recorded, not fixed. Quick 94 / 964 (+1 todo) twice (`+0 / +0`), check 654, e2e 86 / 101 (`+0 / +0`), chunk c4 15 green, allowlist 0, six circles. No string moved from the ledger; no test re-aimed. `docs/TESTING.md`'s Phase 13.1 section, its last subsection. |
