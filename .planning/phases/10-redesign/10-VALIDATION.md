...
| **10-13.2** | **14** | **+0** | **+2** | **not run** | **+2** | `instrument.spec.ts` **5 -> 6** (scan 6: the lattice ground rule, its `:where()` specificity, its source order against `.pill`, and its three declared exceptions read out of three files) and `aesthetic.spec.ts` **7 -> 8** (scan 8: the unlit-cell wash in the token, capped below the dot, with `DARK_BY_CONSTRUCTION` imported rather than quoted). **No spec file is created**, so the files column is 0 and `BASE_FILES + 7` does not move. **Inserted 2026-09-09 by D-22** at wave 14 so nothing renumbers; **`10-14` keeps its number, its wave and its `depends_on`** - unlike D-17's insert, which moved 10-14 to wave 15, this one moves no line of it at all. The sweep is not run and the plan proves it with a PRECISE grep: `grep -n "app\.css|PadFrame|CatalogCard|browse/+page|\.dots|\.lattice"` over the three sweep files exits 1 with no output. **A loose `grep -c "lattice"` matches all three, because `lattice` is a CATALOG ENTRY ID** - recorded because that is a false positive a hurried check would have believed. **The e2e term is +2, and it is the first non-zero one since 10-13**: one `@webkit` title in `aesthetic.e2e.ts` running in both projects. A-55 and A-58 are PAINT-ORDER facts and a source scan is structurally blind to paint order - which is how 10-13.1 shipped a lattice that painted nothing with every scan green. That makes 10-13.2 the **seventh** e2e wave, and the Sampling Rate line is amended by name for the second time |
| **Phase total** | | **+7** | **+48** | `4 19`, **+1 restructure** | **+14** | reconciled at 10-14, never asserted before it, and **re-derived from the per-file table below rather than from the plans' own `check-counts` lines**: 6+1+0+6+5+4−1+0+3+6+4+5+2+5+2+0 = **48**, in **sixteen** terms (the last is 10-14 itself, which writes no tests and is written out rather than omitted, so the term count is the plan count). **It was `+6` / `+40` in fourteen terms until 2026-09-08**, when D-17 inserted 10-13.1 and made it `+7` / `+45` in fifteen; **later the same day D-19 to D-21 moved two of those fifteen terms** — 10-12 `+3` → `+5` and 10-13 `+3` → `+2` — making it `+46`. **On 2026-09-09 D-22 inserted 10-13.2**, also at wave 14 and also without renumbering anything, at **+0 files / +2 tests / +2 e2e** — making it `+48` in sixteen. **The file column has still not moved**: `constants.spec.ts` already existed and 10-13.2 creates no spec file. **The e2e column HAS**, for the first time since the CLEAR fold. The e2e total is `BASE_E2E + 14` in **six** terms: aesthetic 4×2 (10-04), **10-05 0**, **10-07 0**, install 2×2 (10-13), **10-13.1 0**, aesthetic test 5 1×2 (10-13.2). **The seven created files** are `aesthetic`, `font-assets`, `demo`, `facets`, `mix`, `colour-picker`, `instrument`. **Six occurrences of a superseded phase total survive in shipped documents and are deliberately NOT edited**, because a shipped SUMMARY is a record of what was observed and believed and rewriting one is falsifying it: `10-09-PLAN.md:79` and `:226` (`+40`, `+41`), `10-09-SUMMARY.md:178` and `:634` (`+40`), `10-10-SUMMARY.md:175` and `:619` (`+40`). All six predate D-17's insertion of 10-13.1 and were already stale at `+45` before the CLEAR fold, and at `+46` before D-22's. **`10-13.1-PLAN.md:167`'s `BASE_TESTS + 45 -> + 46` is a SEVENTH such occurrence and is also left standing**: it is a true statement about what the CLEAR fold did on the day it was written, not a claim about the phase total, and one dated sentence was appended beside it pointing here instead. **This row is the one place the phase total is stated, and a grep for `BASE_TESTS +` that lands on any of those seven should land here next** |
...
| File | Tests | Plan |
|---|---|---|
| `src/lib/ui/aesthetic.spec.ts` | **1**, then **7**, then **8** | 10-01 (created), 10-04, 10-13.2 |
| `src/lib/ui/instrument.spec.ts` | **5**, then **6** | 10-13.1 (created), 10-13.2 |
| `src/lib/ui/font-assets.spec.ts` | **5** | 10-01 (created) |
| `src/lib/ui/identity.spec.ts` | 6 → **7** | 10-02 |
| `e2e/aesthetic.e2e.ts` | **4**, then **5** titles, both projects | 10-04 (created), 10-13.2 |
...
- **After every wave:** `npm run check 2>&1 | grep -Ei "error|warning"`, `npm run lint`,
  ...
  **`npm run test:quick` again**, because `config-shape.spec.ts` test 14 and `og/build.spec.ts` read
  `build/`.
- **e2e runs in SEVEN waves** — 10-04, 10-05, 10-07, 10-13, **10-13.1**, **10-13.2** and 10-14 — at `--workers 3`,
  ...
  **Amended again 2026-09-09 (D-22).** 10-13.2 is the seventh, and it is the first of the extra two
  with a NON-ZERO delta: `+2`, one `@webkit` title in `aesthetic.e2e.ts` running in both projects.
  Its reason is stronger than 10-13.1's rather than the same one - A-55 and A-58 are both PAINT-ORDER
  facts, and a source scan is structurally blind to paint order. That is exactly how 10-13.1 shipped a
  lattice which painted nothing with every scan green, and the browser gate is the repair.
...
| 10-14-02 | 14 | 15 | all | phase gate | quick `BASE_FILES + 7` / `BASE_TESTS + 48` written as a **sixteen-term** chain, sweep `4 19` restructured, e2e `BASE_E2E + 14` in **six** terms, against a fresh production build; **ROADMAP's six Phase 10 success criteria walked one at a time**, criterion 1 recorded as discharged into `THIRD-PARTY.md` with the resolution handed to the user as Open item 2; `git diff --stat HEAD -- src/vendor/` empty; the archive listed and the font absent from it | exists | pending |
...
- [ ] The phase gate green against a production build at quick `BASE_FILES + 7` / `BASE_TESTS + 48`,
      written as a **sixteen-term** chain, sweep `4 19` and e2e **`BASE_E2E + 14`** in **six** terms;
