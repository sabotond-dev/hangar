...
| 10-12-02 | 12 | 12 | SAFE-01 | unit | `install-copy.ts`: **9** labels, **13** utterances, **7** failure builders unchanged, **4** caps; `WRITE_CLICKS` length 4 and equal to the four control labels; `install-copy.spec.ts` **6**; `session.spec.ts` test 15's needles **10** with the file still at **21**; `install.spec.ts` test 4 counts CLEAR's class | exists | pending |
