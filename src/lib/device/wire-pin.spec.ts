...
describe("the wire pin: the bytes are the numbers (D-10, D-17)", () => {
  ...
  it("for every compiler-driven entry, the published strings are the compiled text and their lengths are the meter's used figures", async () => {
    ...
    // SEVEN since change 12b (2026-09-18): the `tpad` preset is on the shelf
    // but not in the catalog - the hand-authored TRACKPAD replaced it as the
    // card, plan 12-10 - and so is `radar` since the hand-authored RADAR took
    ...
    // cards, pinned by the Lua walk below.
    expect(
      presets,
      "the five carded shelf presets at least",
