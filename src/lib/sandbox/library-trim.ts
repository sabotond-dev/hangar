...
// name - never a second copy of the Lua. 255/0 keeps the head, the map, `U E X N` and the join
// call (what the runtime calls, and what `E` needs); 255/6 keeps the marker alone: the runtime
...
/** The parts 255/0 keeps, by library.ts's names, in its order. */
...
/**
 * 255/0 trimmed, joined as library.ts joins the full string (the head and the
 * map with one space, the map and the functions concatenated, the functions
 * and the call space-joined) so the text is the minifier's own fixed point -
 * runtime.spec.ts proves it canonical and runs it.
 */
export const TRIMMED_LIBRARY: string =
  partNamed("header and tables") +
  " " +
  partNamed("the map") +
  ["U", "E", "X", "N"].map(partNamed).join(" ") +
  " " +
  partNamed("the call");
