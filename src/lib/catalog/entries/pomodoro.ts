...
// Every tap sends a transport note an octave below the alarm; the alarm sounds once at zero.
// Knobs: @MINS (15 20 25 50 1 5, in that order), @RINGC, @BREAKC, @NOTE, @CH. Setup 743 of 908
// at the picker corner (733 at the defaults), Timer 659 (647); restsBlack false. Kind "lua":
// nothing in PadState counts. A kitchen timer, not a stopwatch: every tick is 10 ms late at best.
...
//
// WHAT IT SENDS
//   transport  s:gms(@CH,144,@NOTE-12,90,0) then s:gms(@CH,128,@NOTE-12,0,0) on every accepted
//              tap - the card's only way of saying it started (lua-smoke.spec.ts test 2 fails an
//              entry with no output in 218 ticks); the octave gap is SNAKE's death-note idiom.
//   alarm      s:gms(@CH,144,@NOTE,110,0) then s:gms(@CH,128,@NOTE,0,0), ONCE, at zero.
//
// TRAPS
...
//   - THE NOTE RANGE IS @NOTE-12..@NOTE, 36..84, inside 0..127 at every position; @CH is at all
//     four gms sites. @RINGC and @BREAKC are EXACTLY NINE CHARACTERS EACH (six and three sites).
...
export const POMODORO: CatalogEntry = {
  ...
  knobs: [
    ...
    },
    {
      ...
      kind: "note",
      token: "@NOTE",
      // The alarm, sent once at zero with its note-off; every tap sends this MINUS TWELVE. Two
      // digits at every position; the range is 36..84.
      values: ["60", "72", "48", "84"],
      default: 0,
    },
    {
      ...
      kind: "mode",
      token: "@CH",
      // ZERO-BASED, the first argument of gms. Four channels, not sixteen. At ALL FOUR gms
      // sites, so no note-off goes out on a channel its note-on did not.
      values: ["0", "1", "9", "15"],
      default: 0,
    },
  ],
