...
// catalog's only `txma` / `tyma`. No Timer (`timer: ""`, `static`). Knobs: @CC (the pair is @CC
// and @CC+1), @CH, @BARC, @XFC (id `vernier`), @RAILC. Setup 875 of 908 at the picker corner (857
// at the defaults), Timer 0; restsBlack false. Kind "lua": `sends.faders` is typed `3 | 4`.
...
//     otherwise, both layers); the phases of rows 0..7 are written to 255 once in Setup and
//     never again (folding them into the repaint would double the calls per cell under a moving
...
//
// WHAT IT SENDS
//   s:gms(@CH,176,@CC,v,0)   the fader, on a change of v
//   s:gms(@CH,176,@CC+1,p,0) the crossfader, on a change of p
//
// TRAPS
...
//     becomes 0, silently.
//   - @CC + 1 < 128 AT EVERY KNOB VALUE: 1, 7, 11, 16 (unchanged from the fourteen-bit card so
//     an old stamp decodes to the same four controllers).
...
//   - THE RACK IS UNCHANGED BY THE REWRITE (five knobs, four values each, the same ids), so the
//     shape character does not move.
  ...
  knobs: [
    {
      ...
      kind: "amount",
      token: "@CC",
      // TWO adjacent controllers, @CC and @CC + 1, so every value must satisfy @CC + 1 < 128.
      // Unchanged from the fourteen-bit card so an old stamp decodes to the same numbers.
      values: ["1", "7", "11", "16"],
      default: 0,
    },
    {
