...
these numbers, so a wrong one stays green forever. **Every other row in this table is still its
original wave's measurement, phase 11 has since changed the Lua of several of them, and none of them
has been re-checked** — recorded as `D-11-10-b` in `deferred-items.md`.
