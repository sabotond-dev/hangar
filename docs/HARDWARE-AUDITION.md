...
**SHUTTLE was RE-MEASURED by `11-12-SUMMARY.md` and its row now reads 540 / 538.** Plan 11-12
...
| 18  | **SHUTTLE**       | **(a)** Scrub in a video editor at each of the four speeds, in both directions. Do the keystrokes keep up, does the transport actually reverse, and does the speed the bar shows match how fast the picture is moving? **(b) PICK THE MODULE UP COLD AND SAY WHAT IT DOES BEFORE ANYONE TELLS YOU.** Without being told: which side runs the video forward, where is zero, and what happens when you lift your finger? Then push a speed, lift, and stop it. Note whether you looked for the stop or found it, and how long each answer took. | The same HID invisibility as row 17, plus the 256-byte per-cycle protocol buffer: `gks` costs 10 + 4n bytes and one 10 ms cycle holds about sixty-one key steps, so whether the module can press a key as fast as the top speed asks is a firmware question a simulator cannot answer. **(b) is the bench row plan 11-12 owes and no gate can take from it.** The note this card was rebuilt for was _don't understand how it works_, which is a judgement a person makes at a pad; the rewrite was measured, but whether it ANSWERED the note is not a claim any test in this repository is able to make. Planted and proved in 11-12: giving forward and reverse the same colour — a pad on which the direction cannot be read at all — turns nothing red once `frames.json` is regenerated. |
