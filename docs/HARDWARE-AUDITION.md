...
SONAR — and Phase 9 is adding twenty more, of which HOLD, STEPS and SLAM are the first three.
...
left alone for fifteen minutes. Fifteen rows, each with the reason it belongs to a bench and not to a
test suite.
...
4. **MORPH and SLAM are the exceptions that prove it** — neither has a Timer at all, the Timer event
   of each is the empty string, so both are Setup only, and they are the two cards that start from
   the Setup alone.
5. **Have somewhere to write fifteen lines.** The results go back into this document under a dated
...
`.tmp-audition/<id>.timer.lua` for every one that has a Timer — ten Setup files and eight Timer
...
## The ten, and what they cost

Measured at their default knob positions with the pinned minifier. The first seven come from
`08-06-SUMMARY.md`; HOLD, STEPS and SLAM were measured by `09-03-SUMMARY.md`:
...
**MORPH and SLAM are Setup-only, and that is legitimate rather than an omission.** Both animate only
under a finger, with a per-touch decay that firmware runs down to black on its own, so there is
nothing for a Timer to advance. Store nothing into event 6 for either of them; row 1's install-order
rule below does not apply to a configuration that has no Timer at all.
...
Fifteen rows, in order. Each names why it cannot be simulated, so no row is busywork.
