# WHEELS - the history behind src/lib/catalog/entries/wheels.ts

WHEELS is the pitch wheel on the left and the mod wheel on the right, built so that the two are
not the same control: fourteen-bit bend that springs home on the wire against a seven-bit CC that
stays, a travelling row against a filling bar, a middle-row rest against a zero rest. Its source
is `src/lib/catalog/entries/wheels.ts`. 11-15 authored it from the user's ask, costed eleven
sketches as Lua before a line was written (1118 / 1035 / 922 / 895) and priced four rejected
shapes and four taken levers by removal; the entry's own header now carries the mechanism, the
wire and the traps, and everything the header said before 13.2-02 - the ask, the sketch ladder,
the three-corner table, the `self.D` finding and the reasoning - is below, verbatim.

## Moved from src/lib/catalog/entries/wheels.ts on 2026-09-13 (13.2-02)
