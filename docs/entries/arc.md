# ARC - the history behind src/lib/catalog/entries/arc.ts

ARC is the modulation LFO drawn with a finger: rate left-right, depth up-down, a triangle sent
as a CC at 50 Hz after the finger lifts, the swirl's speed the rate and the heart's pulse the
depth. Its source is `src/lib/catalog/entries/arc.ts`. 11-02 repaired the class-B guard; 11-09
scaled the heart by the depth; 11-09.1 added the stop / resume tap on the centre cell (390 / 262
to 523 / 275 at the picker corner); 12-05 gated the swirl re-arm on the running flag (541);
12.1-03 moved the stop target to the calibrated `N(x,y)==40` (528). The entry's own header now
carries the mechanism, the wire and the traps; everything the header said before 13.2-02 - the
amplitude measurements, the three gestures and two pictures costed, the negative checks and the
budget ladder - is below, verbatim.

## Moved from src/lib/catalog/entries/arc.ts on 2026-09-13 (13.2-02)
