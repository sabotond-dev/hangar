# STAGE - the history behind src/lib/catalog/entries/stage.ts

STAGE is nine scenes for a stream over plain keystrokes: nine four-corner boxes, the live one
glowing, the one being lined up breathing, a `gks` call per cut. Its source is
`src/lib/catalog/entries/stage.ts`. 11-02 declared its class-B exception; 11-09 built the
lined-up state the copy had promised since Phase 9 (the slide chosen, three gestures rejected by
measurement; 518 / 109 to 653 / 136 at the picker corner); 11-16 corrected the corner the header
quoted. The entry's own header now carries the mechanism, the wire and the traps; everything the
header said before 13.2-02 - the gesture costings, the four-corner table, the state measurements
and the declared-exception reasoning - is below, verbatim.

## Moved from src/lib/catalog/entries/stage.ts on 2026-09-13 (13.2-02)
